...
public class CirSim implements MouseDownHandler, MouseMoveHandler, MouseUpHandler, ClickHandler, DoubleClickHandler,
    ...
    TopMenu topMenu;
    ElementContextMenu elmContextMenu;
	...
	layoutPanel.addNorth(topMenu.menuBar, MENUBARHEIGHT);
	...
	elmContextMenu = new ElementContextMenu(localizationMap);
	...
	if (scopeSelected != -1) {
		...
		elmContextMenu.scopeMenuItem.setEnabled(mouseElm.canViewInScope());
		elmContextMenu.floatScopeMenuItem.setEnabled(mouseElm.canViewInScope());
		elmContextMenu.editMenuItem.setEnabled(mouseElm.getEditInfo(0) != null);
		elmContextMenu.flipMenuItem.setEnabled(mouseElm.getPostCount() == 2);
		elmContextMenu.splitMenuItem.setEnabled(canSplit(mouseElm));
		elmContextMenu.sliderMenuItem.setEnabled(sliderItemEnabled(mouseElm));
		contextPanel = new PopupPanel(true);
		contextPanel.add(elmContextMenu.menuBar);
	...
	if ((scopeSelected != -1 && scopes[scopeSelected].cursorInSettingsWheel()) || (scopeSelected == -1
	...
	topMenu.recoverItem.setEnabled(false);
    }

    void enableUndoRedo() {
	topMenu.redoItem.setEnabled(redoStack.size() > 0);
	topMenu.undoItem.setEnabled(undoStack.size() > 0);
    }
	...
	topMenu.pasteItem.setEnabled(clipboard != null && clipboard.length() > 0);
    }
    ...
    public native boolean weAreInUS() /*-{
		try {
			l = navigator.languages ? navigator.languages[0]
    ...
    public native boolean weAreInGermany() /*-{
		try {
			l = navigator.languages ? navigator.languages[0]
					: (navigator.language || navigator.userLanguage);
    ...
    native void printCanvas(CanvasElement cv) /*-{
		...
		var win = window.open("", "print",
				"height=500,width=500,status=yes,location=no");
	...
	topMenu.menuBar.addItem(LS("Circuits"), currentMenuBar);
